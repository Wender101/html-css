                            ...
                            btnsAtualizarNoticia.addEventListener('click', () => {
                                ...
                                function atualizarNoticia(veracidade = '') {
                                    if(noticiaEditada == false) {
                                        ...
                                        let cloneNoticias = UsersMigueley.Noticias
                                    
                                        if(btnsAtualizarNoticia.innerText == 'Aprovar') {
                                            cloneNoticias[c].EstadoNoticia = 'No Ar'
                                            cloneNoticias[c].VeracidadeDaNoticia = veracidade
                                            cloneNoticias[c].Motivo = ''
                                            db.collection('UsersMigueley').doc(valor.id).update({Noticias: cloneNoticias})
                                            
                                        } else if(btnsAtualizarNoticia.innerText == 'Suspender') {
                                            cloneNoticias[c].EstadoNoticia = 'Suspenso'
                                            cloneNoticias[c].VeracidadeDaNoticia = veracidade
                                            cloneNoticias[c].Motivo = document.querySelector('#textAreaMotivoSuspender').value
                                            db.collection('UsersMigueley').doc(valor.id).update({Noticias: cloneNoticias})
