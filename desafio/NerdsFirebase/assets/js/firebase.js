...
function construirProduto(classe, nome, desc, imagem1, imagem2 = imagem1) {
...
function adicionarProduto() {
    ...
    if(nomeProduto == '' || descProduto == '' || linkImg1 == '') {
        ...
        addNoBancoDeDados(select, nomeProduto, descProduto, linkImg1, linkImg2)
        document.getElementById('addProduto').style.display = 'none'
    }
...
function addNoBancoDeDados(classe, nome, desc, imagem1, imagem2) {
    let objProdutos = {
        classe,
        imagem1,
        imagem2,
...
db.collection('Produtos').onSnapshot((data) => {
    ...
    data.docs.map(function(val) {
        let p = val.data()
        construirProduto(p.classe, p.nome, p.desc, p.imagem1, p.imagem2)
    })
}) 
